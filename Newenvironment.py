...
class newenvironment():
    def __init__(self, uid, password, pid, deviceid, port, o_username, o_password, wxmm, phmode, wxmc, phonenumber, gj_mode, tm=None, cooperator=None, country=None,gj=None,qh=None,ip=None):
        ...
        self.ip = ip
        self.w = WB(deviceid)
        self.element_json = json.loads(file.read_all('6.7.2.json'))
    ...
    def input_text(self):
        ...
        self.visualization('获取到手机号码:%s' % self.phonenumber[0])
    ...
    def waiting_code(self, end_time):
        ...
        for self.i in range(1, int(end_time)):
            try:
                ...
                self.visualization('正在发送信息:%s' % self.dx)
    ...
    def receive_Verification_Code(self):
        if self.phmode == '4.国外私人1'.decode("utf-8"):
            return self.tk.get_baji_dx(self.phonenumber[0])
        if self.phmode == '5.国外私人2'.decode("utf-8"):
            self.switching_VPN()
            os.system('adb -s %s shell am start -n com.tencent.mm/.ui.LauncherUI' % self.deviceid)
            return self.tk.get_tkt_yzm(self.phonenumber[0], self.phonenumber[1])
        if self.phmode == '6.国外私人3'.decode('utf-8'):
            return self.tk.gwsr3_send_text(self.phonenumber[1])
        if self.phmode == '11.菜鸟国外'.decode('utf-8'):
            return self.tk.cn_gw_get_dx(self.phonenumber[1])

    def input_Verification_Code(self):
        self.driver.implicitly_wait(300)
        self.driver.find_element_by_id(self.element_json[u'输入框ID']).click()
        dx = self.receive_Verification_Code()
        self.driver.find_element_by_id(self.element_json[u'输入框ID']).send_keys(dx)
    ...
    def xr_wechat(self,wxid=None,cloudCode=None,mf=None):
        ...
        if self.gj_mode == '2.神奇改机'.decode("utf-8"):
            wechat_list = '%s_%s  %s  %s  %s  dd_cloud:%s  %s\n' % (self.phonenumber[0], self.wxmm, self.ip, self.deviceid, datetime.datetime.now().strftime('%Y-%m-%d'), str(self.device_token),mf)
            file().write(wechat_list,'微信账号数据.txt')
        if self.gj_mode == '1.微霸改机'.decode("utf-8"):
            wechat_list = '%s_%s  %s  %s  %s  %s  %s|  %s\n' % (self.phonenumber[0], self.wxmm, self.ip, self.deviceid, datetime.datetime.now().strftime('%Y-%m-%d'), wxid,cloudCode,mf)
            file().write(wechat_list, '微信账号数据.txt')
        if self.gj_mode == '测试'.decode("utf-8"):
            wechat_list = '%s_%s  %s  %s  %s  %s  %s|\n' % (self.phonenumber[0], self.wxmm, self.ip, self.deviceid, datetime.datetime.now().strftime('%Y-%m-%d'), wxid,cloudCode )
            file().write(wechat_list, '微信账号数据.txt')
            ...
            while True:
                ...
                if self.driver.find_elements_by_id(self.element_json[u'错误弹窗确定ID']) != []:
                    ...
                    if '逻辑' in self.cw.encode('utf-8'):
                        ...
                        if self.driver.find_elements_by_id(self.element_json[u'错误弹窗确定ID']) != []:
                            self.cw = self.driver.find_element_by_id(self.element_json[u'错误弹窗内容ID']).get_attribute(
                                ('text'))
                            if '表情' in self.cw.encode('utf-8'):
                                self.driver.find_element_by_name('取消').click()
                                self.save_wechat_data()
    ...
    def sandbox_save(self,wxid):
        ...
        boxbackup = re.findall('(.*?).zip', os.popen('adb -s %s shell ls /sdcard/boxbackup/' % self.deviceid).read())
        old_list = []
        for i in boxbackup:
        ...
        time.sleep(30)
        new_list = re.findall('(.*?).zip', os.popen('adb -s %s shell ls /sdcard/boxbackup/' % self.deviceid).read())
        if len(old_list) != len(new_list):
            self.new_package = new_list[0]
        self.xr_wechat(wxid,self.new_package+'.zip')
        self.visualization('注册数据已写入文件')
        logging.info(self.deviceid + u'-注册数据已写入文件')
        folder = os.path.exists('./package')
        if not folder:
            os.makedirs('./package')
        else:
            pass
        os.popen('adb -s %s pull /sdcard/boxbackup/%s package/%s.zip'%(self.deviceid,self.new_package,self.new_package))
        f = zipfile.ZipFile("./package/%s.zip"%self.new_package, 'r')
        for file in f.namelist():
            f.extract(file, "temp/%s"%self.new_package)
        file_data = open('./temp/%s/0/hook/data.json'%self.new_package, 'r').read()
        self.CPU_ABI = json.loads(file_data)['CPU_ABI']
        self.ANDROID_ID = json.loads(file_data)['android_id']
        A16_list = []
        for filename in os.listdir(r'./temp/%s/0/com.tencent.mm/files/kvcomm'%self.new_package):
            with open('./temp/%s/0/com.tencent.mm/files/kvcomm/%s' % (self.new_package,filename), 'r') as f:
                A16 = re.findall(',(A[0-9a-z]{15})', f.read())
                if A16 != []:
                    A16_list.append(A16[0])
