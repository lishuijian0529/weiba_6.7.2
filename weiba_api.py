...
if __name__ == '__main__':
    #os.popen('adb -s  db9c0d81 shell am start com.miui.miuibbs/.MainActivity')
    #os.popen('adb  shell /system/bin/screencap -p /sdcard/Download/succ.png')
    deviceid='db9c0d81'
    ...
    # h = open("./%s/%s.png" % (deviceid, deviceid), "wb")
    # h.write(base64.b64decode(image))
    # h.close()
    # os.popen('adb -s %s push ./%s/%s.png  /sdcard/myData/%s.png' % (
    # deviceid, deviceid, deviceid, deviceid)).read()
    # time.sleep(2)
    # os.popen('adb -s %s shell mv /sdcard/myData/%s.png /sdcard/myData/scan.png' % (deviceid, deviceid)).read()
    # time.sleep(2)
    a = WB('db9c0d81').scancode()
